# Skills Collection

This repository contains a collection of skills for AI agents.

## Skills

| Skill | Description | License |
|-------|-------------|---------|
| [agent-browser](./agent-browser) | Browser automation CLI for AI agents | Apache 2.0 |
| [browser-use](./browser-use) | Browser automation for web testing and data extraction | MIT |
| [find-skills](./find-skills) | Discover and install agent skills | MIT |
| [notion-second-brain](./notion-second-brain) | Notion second brain integration | - |
| [skill-creator](./skill-creator) | Guide for creating effective skills | MIT |
| [using-git-worktrees](./using-git-worktrees) | Create isolated git worktrees | MIT |

---

## Attribution

The following skills were created by others and are included here with their respective licenses:

### agent-browser
- **Source**: [vercel-labs/agent-browser](https://github.com/vercel-labs/agent-browser)
- **Author**: Vercel Inc.
- **License**: Apache License 2.0
- **Copyright**: Copyright 2025 Vercel Inc.

### browser-use
- **Source**: [browser-use/browser-use](https://github.com/browser-use/browser-use)
- **Author**: Gregor Zunic
- **License**: MIT License
- **Copyright**: Copyright 2024 Gregor Zunic

### find-skills
- **Source**: [Yeachan-Heo/oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode)
- **Author**: Yeachan Heo
- **License**: MIT License
- **Copyright**: Copyright 2025 Yeachan Heo

### skill-creator
- **Source**: [Yeachan-Heo/oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode)
- **Author**: Yeachan Heo
- **License**: MIT License
- **Copyright**: Copyright 2025 Yeachan Heo

### using-git-worktrees
- **Source**: [tbdng/worktrees-skill](https://github.com/tbdng/worktrees-skill)
- **Author**: Tobia De Angelis
- **License**: MIT License
- **Copyright**: Copyright 2025 Tobia De Angelis

---

## License

Each skill has its own LICENSE file. See individual skill directories for details.
